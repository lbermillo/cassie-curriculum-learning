...
class CassieEnv:
    def __init__(self, traj, simrate=60, clock_based=False, state_est=False, target_action_weight=1.0, reward_cutoff=0.3, max_speed=10, config="cassie/cassiemujoco/cassie.xml"):
        ...
        if clock_based:
            self.observation_space = np.zeros(42 + 1) # add speed input
            if self.state_est:
                self.observation_space = np.zeros(48 + 1)  # Size for use with state est and speed input
        else:
...
#         self.action_space = np.zeros(10)
        self.action_space = gym.spaces.Box(-1.25 * np.ones(10), 1.25 * np.ones(10))
    ...
    def step_simulation(self, action):
        ...
        target = action + (self.target_action_weight * ref_pos[self.pos_idx])
    ...
    def step(self, action):
        ...
        # Early termination
        done = not (0.4 < height < 3.0)

        reward = self.compute_reward()

        if reward < self.reward_cutoff:
            done = True

        return self.get_full_state(), reward, done, {}

    def reset(self, phase=None, speed=None):
            
        self.phase = int(phase) if phase is not None else random.randint(0, self.phaselen)
        self.speed = speed      if speed is not None else (random.randint(0, self.max_speed)) / 10
        
        # print(self.phase, self.speed)
